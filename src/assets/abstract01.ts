...
import { Scene, MeshBuilder, Mesh, Vector4, Vector2, Color3, Vector3, Curve3, StandardMaterial, Texture, NoiseProceduralTexture} from "@babylonjs/core";
...
export class Abstract01 {
	...
	// base mesh: define some parameters
		...
		c_faceUV[2] = new Vector4(0.0, 0.0, c_baseHeight, 1.0); //right side
		c_faceUV[3] = new Vector4(0.0, 0.0, c_baseHeight, 1.0); //left side
		...
		c_faceUV[5] = new Vector4(0.0, 0.0, 1.0, 1.0); //bottom side
        	...
        	//c_base.rotation = new Vector3(0, Math.PI * 0.25, 0);// for debugging
...
//      	};
		...
		// created the extrusion
		const c_vtile = 4;
  		//const c_extruded = MeshBuilder.ExtrudeShapeCustom("extruded", {shape: c_shape, path: c_path, scaleFunction: myScale, rotationFunction: myRotation, closePath: true, closeShape: true, sideOrientation: Mesh.FRONTSIDE, frontUVS: new Vector4(0,1,3,1), });
		const c_extruded = MeshBuilder.ExtrudeShape("extruded", {shape: c_shape, path: c_path, closePath: true, closeShape: true, sideOrientation: Mesh.DOUBLESIDE, backUVs: new Vector4(0,0,1, c_vtile), invertUV: false, updatable: true}, scene);

		// post scale and position
...
// final touch
		...
		//c_mesh_mat.diffuseTexture = new Texture("https://www.babylonjs-playground.com/textures/amiga.jpg");
		c_mesh_mat.diffuseTexture = new Texture("https://www.babylonjs-playground.com/environment/ground.jpg");
		c_mesh_mat.diffuseTexture.wrapU = Texture.WRAP_ADDRESSMODE;
		c_mesh_mat.diffuseTexture.wrapV = Texture.WRAP_ADDRESSMODE;
...
//        	c_mesh_mat.diffuseTexture = c_procTxr
        	...
        	c_mesh_mat.emissiveColor = new Color3(.5, .5, .5) // for debugging the diffuse texture
